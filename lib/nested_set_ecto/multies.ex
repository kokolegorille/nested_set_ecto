defmodule NestedSetEcto.Multies do
  @moduledoc """
  Documentation for NestedSetEcto.Multies
  Returns multies or changeset for crud actions
  """

  defmacro __using__(_opts) do
    quote do
      import NestedSetEcto.Multies
      ...
      def delete(%__MODULE__{lft: lft, rgt: rgt} = resource) do
        ...
        update_rgt_query =
          from(i in __MODULE__, where: i.rgt > ^rgt, update: [inc: [rgt: ^(-width)]])

        update_lft_query =
          from(i in __MODULE__, where: i.lft > ^rgt, update: [inc: [lft: ^(-width)]])
      ...
      def move_to_child_of(
        ...
        case is_or_is_ancestor_of?(resource, parent) do
          true ->
            {:error, "cannot be moved to child of child or self!"}
      ...
      def move_to_right_of(
            %__MODULE__{lft: lft, rgt: rgt, depth: depth} = resource,
            %__MODULE__{rgt: b_rgt, depth: b_depth} = brother
          ) do
        case is_or_is_ancestor_of?(resource, brother) do
          true ->
            {:error, "cannot be moved to right of child or self!"}
          ...
          _ ->
            ...
            [{inside_lft, inside_rgt, inside_inc}, {outside_lft, outside_rgt, outside_inc}] =
              if lft > b_rgt do
                # From right to left
                diff = b_rgt + 1 - lft
      ...
      def move_to_left_of(
            %__MODULE__{lft: lft, rgt: rgt, depth: depth} = resource,
            %__MODULE__{lft: b_lft, depth: b_depth} = brother
          ) do
        case is_or_is_ancestor_of?(resource, brother) do
          true ->
            {:error, "cannot be moved to left of child or self!"}
  ...
  defmacro left_fragment(
    ...
    quote do
      fragment(
        ...
        unquote(inside_lft),
        unquote(inside_rgt),
        unquote(inside_increment),
        unquote(outside_lft),
        unquote(outside_rgt),
        unquote(outside_increment)
      )
    end
  end

  defmacro right_fragment(
             inside_lft,
             inside_rgt,
             inside_increment,
             outside_lft,
